...
본 저장소는 SSAFY AGV 프로젝트의 **Gazebo 시뮬레이션 파트**입니다.

본 시뮬레이션은 실제 AGV의 정밀 좌표를 1:1로 복제하는 완전한 디지털 트윈이 아니라,
...
| 실행 방식         | 설명                                            |
| ------------- | --------------------------------------------- |
| 로컬 시나리오 기반 실행 | 서버 없이 JSON 시나리오 파일을 읽어 AGV01/AGV02와 object 이동 |
...
│   │   ├── scenario_plan_cross_7_9_v3_step23_only.json
│   │   └── object_motion_config_final_step23_v3_preset.json
│   │
│   ├── scripts/
│   │   ├── move_agv_from_scenario_object_final_step23_v4_preset.py
│   │   ├── move_agv_from_api_server.py
│   │   ├── gazebo_launcher.py
...
└── worlds/
    ├── final/
    │   ├── make_agv_world_final_box_agv_v2_revised_v8_stable_objects.py
...
| 폴더                      | 역할                                                 |
| ----------------------- | -------------------------------------------------- |
| `agv_sim/config/`       | 시뮬레이션 경로, 구역 매핑, 전체 시나리오, object 이동 설정 JSON 저장     |
| `agv_sim/scripts/`      | AGV 이동 실행, 서버 API 연동, Gazebo 실행 연동, ArUco 인식 코드 저장 |
...
| `agv_sim/test/`         | ROS2 패키지 기본 테스트 파일                                 |
...
| 구분           | 파일명                                                            | 역할                                                                                                                  |
| ------------ | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| World 생성     | `make_agv_world_final_box_agv_v2_revised_v8_stable_objects.py` | 최종 Gazebo 공장 MAP을 생성하는 Python 파일. AGV01/AGV02, 검정색 라인, 컨베이어, 자재 보관 구역, 완제품 보관 구역, 입출고 구역, 교차 구역, 카메라, object 모델을 생성 |
...
| 구역 경로        | `zone_route_map_final_cross_7_9_v2_empty_box_return.json`      | `MATERIAL_BOX_STORAGE → CONVEYOR_START` 같은 구역 이동을 waypoint 경로로 변환하는 route map                                       |
| 전체 시나리오      | `scenario_plan_cross_7_9_v3_step23_only.json`                  | AGV01/AGV02의 전체 작업 순서, 상태, 명령, 적재물을 정의한 최종 시나리오 파일. Object-motion 최종 데모 기준으로 step 1~23까지만 사용                        |
| Object 이동 설정 | `object_motion_config_final_step23_v3_preset.json`             | 자재 큐브, 완제품 상자, 빈 상자, 보급 상자의 위치와 이동 규칙을 정의한 파일                                                                       |
| 시나리오 실행      | `move_agv_from_scenario_object_final_step23_v4_preset.py`      | 로컬 JSON 시나리오와 object motion config를 읽어 Gazebo에서 AGV01/AGV02 및 object를 단계별로 이동시키는 최종 실행 코드                           |
| 서버 API 연동    | `move_agv_from_api_server.py`                                  | 서버 API에서 AGV 상태와 위치를 받아 Gazebo AGV 위치에 반영하는 코드                                                                      |
...
### 9-1. `scenario_plan_cross_7_9_v3_step23_only.json`
...
### 9-2. `zone_route_map_final_cross_7_9_v2_empty_box_return.json`
...
### 9-3. `line_path_map_final_v6_cross_7_9_names.json`
...
`line_path_map_final_v6_cross_7_9_names.json`의 waypoint는 실제 ArUco 마커가 아니라,
**Gazebo 시뮬레이션에서 AGV가 따라갈 가상 경로점**입니다.

---

### 9-4. `object_motion_config_final_step23_v3_preset.json`

AGV가 운반하는 object의 위치와 이동 규칙을 정의합니다.

관리하는 대상 예시:

| object                | 의미                      |
| --------------------- | ----------------------- |
| `obj_supply_box_01`   | 보급용 자재 상자               |
| `obj_finished_box_01` | 완제품 상자                  |
| `obj_red_cube_03`     | CHIP 또는 완제품 구성 object   |
| `obj_blue_cube_03`    | SENSOR 또는 완제품 구성 object |

이 파일은 다음 정보를 관리합니다.

| 항목                   | 의미                             |
| -------------------- | ------------------------------ |
| `positions`          | object가 놓일 수 있는 주요 위치 좌표       |
| `payload_offsets`    | AGV 위에 object를 올렸을 때 상대 위치     |
| `object_groups`      | 여러 object를 하나의 묶음으로 관리하기 위한 그룹 |
| `fixed_object_poses` | object가 AGV 위에 없을 때 고정될 위치     |
| `conveyor_paths`     | 컨베이어 위 object 이동 경로            |

Object 이동은 실제 집게/물리엔진 기반 픽업이 아니라,
시나리오 상태에 따라 object model pose를 변경하는 방식입니다.
...
`marker_map.json`은 위치 인식/설명용 기준표이고,
AGV의 실제 이동은 `line_path_map_final_v6_cross_7_9_names.json`을 기준으로 수행합니다.
...
gazebo --verbose ~/gazebo_agv_ws/src/agv_sim/worlds/agv_factory_final_box_agv_v2.world
```

---

### 10-2. 최종 로컬 시나리오 기반 실행

서버 없이 최종 시나리오와 object 이동을 확인할 때 사용합니다.

```bash
cd ~/gazebo_agv_ws/src/agv_sim/scripts

python3 move_agv_from_scenario_object_final_step23_v4_preset.py \
  --scenario-plan scenario_plan_cross_7_9_v3_step23_only.json \
  --zone-route-map zone_route_map_final_cross_7_9_v2_empty_box_return.json \
  --line-path-map line_path_map_final_v6_cross_7_9_names.json \
  --object-motion-config object_motion_config_final_step23_v3_preset.json \
  --speed 4.0
```
...
```bash
cd ~/gazebo_agv_ws/src/agv_sim/scripts
python3 move_agv_from_api_server.py
```
...
Gazebo world 실행
    ↓
move_agv_from_api_server.py 실행
    ↓
서버 상태 기반 AGV 이동
```
...
`move_agv_from_api_server.py`는 서버에서 받은 `status`에 따라 다음과 같이 처리합니다.
...
* `line_path_map_final_v6_cross_7_9_names.json`의 waypoint는 실제 ArUco 마커가 아니라 시뮬레이션 이동용 가상 경로점입니다.
* 서버 API 기반 실행 시 `current_position`, `next_position`은 Gazebo 좌표 기준이어야 합니다.
* 서버 연동이 불안정할 경우 `move_agv_from_scenario_object_final_step23_v4_preset.py`로 로컬 시나리오 실행이 가능합니다.
* Object 이동은 실제 집게/물리엔진 기반 픽업이 아니라, 시나리오 상태에 따라 object model pose를 변경하는 방식입니다.
...
최종 로컬 시나리오는 scenario_plan, zone_route_map, line_path_map, object_motion_config를 기반으로 실행됩니다.
서버 연동 시에는 move_agv_from_api_server.py가 서버 상태를 읽어 AGV 위치를 반영합니다.
